...
const handleDbUpdate = async (
    ctx, userId, orderId, status, sdekNumber = null
) =>
{
    try
    {
...
export async function tableUpdateConversation(conversation, ctx)
{
    ...
    const { message: { text: orderRowNumber } } = await conversation.wait()
    ...
    const notifApproved = await statusNotificator(ctx, conversation, userId, orderUniqueId, status, sdekNumber)
